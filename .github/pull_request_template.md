...
For low/standard risk, add one trusted immutable exact-head marker with a nonempty summary:

```text
<!-- foundation-coordinator-review:<exact-sha>:clean -->
```

For protected risk, add both trusted immutable exact-head markers with nonempty summaries:
