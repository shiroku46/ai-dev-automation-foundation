...
def _apply_solution(task_id: str, workspace: Path) -> None:
    if task_id == "foundation.task-001":
        ...
        path = workspace / "tests/test_slug.py"
        ...
        (workspace / "src/display.py").write_text(
        ...
        source = workspace / "src/old_name.py"
        source.rename(workspace / "src/new_name.py")
        ...
        path = workspace / "tests/test_code.py"
        source = path.read_text(encoding="utf-8")
        method = (
...
class InitialEvaluationSuiteTest(unittest.TestCase):
    ...
    def test_catalog_and_new_task_metadata(self):
        raw = CATALOG.read_bytes()
        self.assertEqual(self.loaded.catalog.suite_id, "foundation.initial")
        self.assertEqual(self.loaded.catalog.suite_version, 6)
        self.assertEqual(self.loaded.catalog.foundation_sha, FOUNDATION_SHA)
        self.assertEqual(self.loaded.catalog.task_count, 25)
        self.assertEqual(self.loaded.catalog.catalog_sha256, CATALOG_SHA)
        self.assertEqual(hashlib.sha256(raw).hexdigest(), CATALOG_SHA)
        self.assertEqual(
            tuple(task.entry.task_id for task in self.loaded.tasks),
            tuple(f"foundation.task-{index:03d}" for index in range(1, 26)),
        )
        self.assertEqual(tuple(task.entry.manifest_sha256 for task in self.loaded.tasks), MANIFEST_SHAS)
        expected_new = {
            "foundation.task-021": ("bug_fix", "change_required", "standard"),
            "foundation.task-022": ("test_addition", "change_required", "low"),
            "foundation.task-023": ("protected_boundary", "change_required", "protected"),
            "foundation.task-024": ("stale_evidence", "no_change_required", "standard"),
            "foundation.task-025": ("provider_unavailable", "change_required", "standard"),
        }
        for task_id, expected in expected_new.items():
            manifest = self.by_id[task_id].manifest
            self.assertEqual(
                (manifest.category, manifest.expected_completion_class, manifest.risk_tier),
                expected,
            )
            self.assertEqual(manifest.grader.runtime, "python3.12")
            self.assertEqual(manifest.grader.network_mode, "disabled")
        self.assertIsNone(self.by_id["foundation.task-025"].manifest.expected_human_action_reason)

        protected_ids = {
            task.entry.task_id for task in self.loaded.tasks if task.manifest.risk_tier == "protected"
        }
        self.assertEqual(protected_ids, {"foundation.task-005", "foundation.task-014", "foundation.task-023"})
    ...
    def _run_grader(self, task, workspace: Path, result_path: Path):
        ...
        completed = subprocess.run(
            [sys.executable, str(grader), "--workspace", str(workspace), "--result", str(result_path)],
    ...
    def test_accepted_and_stale_handoff_contracts_remain_enforced(self):
        ...
        accepted = parse_handoff_bundle(
            ...
            expected_repository="shiroku46/eval-handoff",
        ...
        old = parse_handoff_bundle(
            ...
            (root / "decisions.jsonl").read_bytes(),
            (root / "handoff.md").read_bytes(),
            expected_repository="shiroku46/eval-stale-handoff",
            expected_issue_number=15,
            expected_base_sha="5" * 40,
            expected_candidate_sha="6" * 40,
        )
