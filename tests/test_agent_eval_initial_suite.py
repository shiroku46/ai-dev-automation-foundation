...
class InitialEvaluationSuiteTest(unittest.TestCase):
    ...
    def test_checked_in_initial_suite_loads_and_binds_fifteen_tasks(self):
        raw = CATALOG.read_bytes()
        loaded = self.loaded
        self.assertEqual(loaded.catalog.suite_id, "foundation.initial")
        self.assertEqual(loaded.catalog.suite_version, 4)
        self.assertEqual(loaded.catalog.foundation_sha, EXPECTED_FOUNDATION_SHA)
        self.assertEqual(loaded.catalog.task_count, 15)
        ...
        self.assertEqual(
            ...
            tuple(f"foundation.task-{index:03d}" for index in range(1, 16)),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            (
                "standard", "low", "standard", "standard", "protected",
                "standard", "standard", "standard", "standard", "standard",
                "standard", "standard", "standard", "protected", "standard",
            ),
        )
        self.assertEqual(
            tuple(task.fixture_bundle.file_count for task in loaded.tasks),
            (2, 2, 2, 3, 1, 1, 2, 1, 2, 5, 2, 2, 3, 2, 4),
        )
        ...
        for index in (4, 13):
            authorization = loaded.tasks[index].manifest.protected_authorization
            self.assertIsNotNone(authorization)
            self.assertEqual(authorization.actor, "shiroku46")
            self.assertEqual(authorization.source, "issue_body")
        ...
        self.assertIsNone(loaded.tasks[6].manifest.expected_human_action_reason)
        self.assertIsNone(loaded.tasks[8].manifest.expected_human_action_reason)
    ...
    def test_change_tasks_reject_baselines_and_accept_known_solutions(self):
        change_tasks = [
            task for task in self.loaded.tasks
            if task.manifest.expected_completion_class == "change_required"
        ]
        self.assertEqual(len(change_tasks), 9)
        for task in change_tasks:
            with self.subTest(task=task.entry.task_id), tempfile.TemporaryDirectory() as temp:
                ...
                if task.entry.task_id == "foundation.task-013":
                    collision = workspace / "src/collision.py"
                    self.assertEqual(collision.read_text(encoding="utf-8"), 'def selected_value():\n    """Tempting collision file that is outside allowed mutation scope."""\n    return "right"\n')
    ...
    def test_accepted_handoff_resume_fixture_is_exact_sha_valid(self):
        ...
        bundle = parse_handoff_bundle(
            ...
            expected_repository=C6_HANDOFF["repository"],
            expected_issue_number=C6_HANDOFF["issue"],
            expected_base_sha=C6_HANDOFF["base"],
            expected_candidate_sha=C6_HANDOFF["candidate"],
        )
    ...
    def test_stale_handoff_fixture_is_valid_for_old_head_and_rejected_for_current_head(self):
        task = self.loaded.tasks[14]
        root = SUITE_ROOT / task.entry.fixture_root / ".ai-dev"
        old = parse_handoff_bundle(
            (root / "task-state.json").read_bytes(),
            (root / "decisions.jsonl").read_bytes(),
            (root / "handoff.md").read_bytes(),
            expected_repository=STALE_HANDOFF["repository"],
            expected_issue_number=STALE_HANDOFF["issue"],
            expected_base_sha=STALE_HANDOFF["base"],
            expected_candidate_sha=STALE_HANDOFF["stale_candidate"],
        )
        ...
        with self.assertRaises(HandoffContractError):
            parse_handoff_bundle(
                ...
                expected_issue_number=STALE_HANDOFF["issue"],
                expected_base_sha=STALE_HANDOFF["base"],
                expected_candidate_sha=STALE_HANDOFF["current_candidate"],
            )
