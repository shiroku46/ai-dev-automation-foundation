...
def make_job(name, run_id=7, sha=SHA, status="completed", conclusion="success"):
    return {
        "name": name,
        "run_id": run_id,
        "head_sha": sha,
...
class RunJobEvidenceTest(unittest.TestCase):
    ...
    def classify(self, run, jobs):
        with patch.object(self.runtime, "trusted_runs_for_sha", return_value=[run]), patch.object(
            ...
            return self.runtime.attestation_attempts(SHA)[0]
    ...
    def test_unique_successful_required_jobs_authorize(self):
        ...
        self.assertEqual(result, {"run_id": 7, "active": False, "success": True, "complete": True})

    def test_missing_or_duplicate_job_fails_closed(self):
        run = {"id": 7, "status": "completed", "conclusion": "success"}
        missing = self.classify(run, [make_job("CI / validate")])
        duplicate = self.classify(
            run,
            [make_job("CI / validate"), make_job("CI / validate"), make_job("Unit Tests / test")],
        )
        self.assertFalse(missing["success"])
        self.assertFalse(duplicate["success"])

    def test_wrong_sha_or_failed_job_fails_closed(self):
        run = {"id": 7, "status": "completed", "conclusion": "success"}
        wrong_sha = self.classify(
            run,
            [make_job("CI / validate", sha="b" * 40), make_job("Unit Tests / test")],
        )
        failed = self.classify(
            run,
            [make_job("CI / validate", conclusion="failure"), make_job("Unit Tests / test")],
        )
    ...
    def test_active_run_does_not_fetch_jobs(self):
        ...
        with patch.object(self.runtime, "trusted_runs_for_sha", return_value=[run]), patch.object(
            self.runtime, "trusted_run_jobs"
        ) as fetch_jobs:
            result = self.runtime.attestation_attempts(SHA)[0]
