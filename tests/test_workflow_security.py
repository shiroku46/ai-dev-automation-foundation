...
class WorkflowSecurityTest(unittest.TestCase):
    ...
    def test_queue_and_reconciliation_are_fixed_default_branch_paths(self):
        ...
        self.assertIn('expected_path = f".github/workflows/ci-reconcile.yml@{default_branch}"', queue)
        self.assertNotIn('expected_path = f".github/workflows/supervisor.yml@{default_branch}"', queue)
        ...
        for required in (
            ...
            self.assertIn(required, queue_recovery)
        for forbidden in ("secrets.", "id-token: write", "issues: write", "pull-requests: write"):
            self.assertNotIn(forbidden, queue_recovery)
