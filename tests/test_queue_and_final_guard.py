...
class QueueAndFinalGuardTest(unittest.TestCase):
    ...
    def test_manual_sweep_recovery_reuses_branch_or_artifact_before_retry(self):
        recovery = job_block(workflow("ci-reconcile.yml"), "queue_recovery")
        ...
        sweep_path = recovery.split("          else:\n              if not active_queue_run():", 1)[1]
        branch_position = sweep_path.index("resumed = resume_remote_branch")
        artifact_position = sweep_path.index("recovered = latest_verified_artifact")
        retry_position = sweep_path.index("dispatch_retry(issue, base_sha, FailureClass.UNKNOWN, None)")
