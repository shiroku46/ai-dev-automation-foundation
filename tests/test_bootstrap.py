...
def run_queue_materializer(
    ...
    *,
    ...
    candidate = {
        "version": 1,
        "base_sha": base_sha,
        "branch_name": "claude-issue-162-symlink-test",
        "files": files,
    }
    ...
    return subprocess.run(
        [
            sys.executable,
            "-c",
            queue_python_step("Verify digest and materialize complete candidate bytes"),
        ],
        cwd=repository,
        env=environment,
        capture_output=True,
    ...
    *,
    ...
    environment.update(
        {
            "BASE_SHA": base_sha,
            "ISSUE_NUMBER": "162",
            "CLAUDE_BRANCH": "claude-issue-162-symlink-test",
            "RUNNER_TEMP": str(runner_temp),
    ...
    return subprocess.run(
        [
            sys.executable,
            "-c",
            queue_python_step("Package complete candidate bytes and manifest"),
        ],
        cwd=repository,
        env=environment,
        capture_output=True,
...
class BootstrapTest(unittest.TestCase):
    def test_rendered_allowlist_and_target_validation(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn(
                "Allow GitHub Actions to create and approve pull requests", checklist
            )
            ...
            issue_template = (
                target / ".github/ISSUE_TEMPLATE/ai-task.yml"
            ...
            self.assertIn(
                "automation-stops/pr-<number>/<sha>/<REASON>.json", checklist
            )
    ...
    def test_generated_marker_remains_authoritative_when_target_adds_tests(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            source_marker.write_text(
                "# target-specific tests are allowed\n", encoding="utf-8"
            )
    ...
    def test_queue_packager_rejects_untracked_fifo_without_blocking(self):
        with tempfile.TemporaryDirectory() as directory:
            scratch = Path(directory)
            repository = scratch / "repo"
            repository.mkdir()
            ...
            os.mkfifo(repository / "candidate-fifo")
            result = run_queue_packager(repository, scratch, base_sha=base_sha)
