...
17. Human notification is allowed only for the three canonical account/provider UI reason codes after the same connected audit and live destination revalidation. The Phase 0 Workflow-permissions instruction is presented directly as repository startup guidance when no connected settings endpoint exists; it does not create a new runtime reason code.
