...
def render(target: Path, owner: str) -> None:
    ...
    (target / "INSTALL_CHECKLIST.md").write_text(
        ...
        "- [ ] Confirm internal stops are sanitized canonical JSON at `automation-stops/pr-<number>/<sha>/<REASON>.json` and are never posted as Issue or Pull Request comments.\n"
        ...
        "- [ ] Confirm every human-only notice performs the connected audit, revalidates its live destination, and includes exact Issue, Pull Request, SHA, attempted paths, impossibility evidence, canonical UI action, target/provider, and automatic-resumption condition.\n"
        "- [ ] Confirm only immutable `github-actions[bot]` comments can deduplicate a human-only notice.\n"
