...
def render(target: Path, owner: str) -> None:
    ...
    (target / "INSTALL_CHECKLIST.md").write_text(
        ...
        "- [ ] Confirm an owner-authored existing-PR-base block is accepted only after fresh open, non-fork, same-repository ref and exact-SHA verification.\n"
        "- [ ] Confirm exact-base candidates use immutable credential-free checkout, digest-bound full-byte handoff, read-only validation, and Git Data API publication without candidate execution.\n"
        "- [ ] Confirm recovery creates a separate generated branch and Draft integration Pull Request against the verified existing branch; ordinary Issues still target the default branch.\n"
