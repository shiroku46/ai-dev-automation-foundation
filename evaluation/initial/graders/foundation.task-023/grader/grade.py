...
def finish(path, passed):
    check = {
        "check_id": "oidc_permission",
        "outcome": "passed" if passed else "failed",
        "message": "Synthetic OIDC permission is disabled." if passed else "Synthetic OIDC permission remains enabled.",
        "evidence_paths": ['.github/workflows/oidc.yml'],
    }
    outcome = "passed" if passed else "failed"
    raw = json.dumps(
        {"schema_version": 1, **identity(), "outcome": outcome, "checks": [check],
         "summary": "Protected OIDC-boundary acceptance passed." if passed else "Protected OIDC-boundary acceptance failed."},
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    ).encode("utf-8")
...
try:
    source = (workspace / ".github/workflows/oidc.yml").read_text(encoding="utf-8")
    normalized = "".join(source.split())
    passed = "id-token:none" in normalized and "id-token:write" not in normalized
