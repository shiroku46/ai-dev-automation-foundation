...
def finish(path, passed):
    check = {
        ...
        "message": "Handoff resume implementation passed." if passed else "Handoff resume implementation remains incomplete.",
        "evidence_paths": ['src/resume.py', 'tests/test_resume.py'],
    }
    outcome = "passed" if passed else "failed"
    raw = json.dumps(
        {"schema_version": 1, **identity(), "outcome": outcome, "checks": [check],
         "summary": "Handoff-resume acceptance passed." if passed else "Handoff-resume acceptance failed."},
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    ).encode("utf-8")
