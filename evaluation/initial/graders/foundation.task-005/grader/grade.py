...
try:
    source = (workspace / ".github/workflows/check.yml").read_text(encoding="utf-8")
    normalized = "".join(source.split())
    passed = "permissions:contents:read" in normalized and "contents:write" not in normalized
except (OSError, UnicodeError):
    passed = False
raise SystemExit(finish(
    result,
    [{
        "check_id": "workflow_permissions",
        "outcome": "passed" if passed else "failed",
        "message": "Synthetic workflow permissions are read-only." if passed else "Synthetic workflow permissions remain overbroad.",
        "evidence_paths": [".github/workflows/check.yml"],
    }],
    "Protected-boundary acceptance passed.",
    "Protected-boundary acceptance failed.",
))
