...
def finish(path, passed):
    check = {
        "check_id": "quota_fallback",
        "outcome": "passed" if passed else "failed",
        "message": 'Provider quota exhaustion falls back to GitHub-direct.' if passed else 'Provider quota exhaustion still requests human action.',
        "evidence_paths": ['src/route.py', 'tests/test_route.py'],
    }
    outcome = "passed" if passed else "failed"
    raw = json.dumps(
        {"schema_version": 1, **identity(), "outcome": outcome, "checks": [check],
         "summary": 'Provider-quota fallback acceptance passed.' if passed else 'Provider-quota fallback acceptance failed.'},
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    ).encode("utf-8")
