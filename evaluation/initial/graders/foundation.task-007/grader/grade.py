...
def finish(path, passed):
    check = {
        ...
        "message": "Provider-unavailable route falls back to GitHub-direct." if passed else "Provider-unavailable route does not fall back to GitHub-direct.",
        "evidence_paths": ['src/route.py', 'tests/test_route.py'],
    }
    outcome = "passed" if passed else "failed"
    raw = json.dumps(
        {"schema_version": 1, **identity(), "outcome": outcome, "checks": [check],
         "summary": "Provider-unavailable acceptance passed." if passed else "Provider-unavailable acceptance failed."},
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    ).encode("utf-8")
