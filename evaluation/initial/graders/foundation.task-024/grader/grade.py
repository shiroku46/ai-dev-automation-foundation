...
def finish(path, passed):
    check = {
        "check_id": "cross_task_evidence",
        "outcome": "passed" if passed else "failed",
        "message": "Current task state remains unchanged." if passed else "Current task state changed using cross-task evidence.",
        "evidence_paths": ['src/task_state.py', 'evidence/cross-task-result.txt'],
    }
    outcome = "passed" if passed else "failed"
    raw = json.dumps(
        {"schema_version": 1, **identity(), "outcome": outcome, "checks": [check],
         "summary": "Cross-task stale-evidence acceptance passed." if passed else "Cross-task stale-evidence acceptance failed."},
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    ).encode("utf-8")
...
try:
    passed = (workspace / "src/task_state.py").read_text(encoding="utf-8") == 'def task_state():\n    """Return the already-correct task state."""\n    return "complete"\n'
