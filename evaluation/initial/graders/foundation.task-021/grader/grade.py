...
def finish(path, passed):
    check = {
        "check_id": "inclusive_window",
        "outcome": "passed" if passed else "failed",
        "message": "Inclusive boundary behavior passed." if passed else "Inclusive boundary behavior failed.",
        "evidence_paths": ['src/window.py', 'tests/test_window.py'],
    }
    outcome = "passed" if passed else "failed"
    raw = json.dumps(
        {"schema_version": 1, **identity(), "outcome": outcome, "checks": [check],
         "summary": "Inclusive-window bug-fix acceptance passed." if passed else "Inclusive-window bug-fix acceptance failed."},
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    ).encode("utf-8")
