...
def finish(path, passed):
    check = {
        "check_id": "exact_rename",
        "outcome": "passed" if passed else "failed",
        "message": "Exact rename completed with old path removed." if passed else "Exact rename is incomplete or left the old path.",
        "evidence_paths": ["src/new_name.py", "tests/test_rename.py"],
    }
    outcome = "passed" if passed else "failed"
    raw = json.dumps(
        {"schema_version": 1, **identity(), "outcome": outcome, "checks": [check],
         "summary": "Exact-rename acceptance passed." if passed else "Exact-rename acceptance failed."},
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    ).encode("utf-8")
...
renamed = (workspace / "src/new_name.py").is_file() and not (workspace / "src/old_name.py").exists()
raise SystemExit(finish(result, renamed and unit_tests(workspace)))
