...
def finish(path, passed):
    check = {
        "check_id": "timeout_fallback",
        "outcome": "passed" if passed else "failed",
        "message": "Provider timeout falls back to GitHub-direct." if passed else "Provider timeout still requests human action.",
        "evidence_paths": ['src/route.py', 'tests/test_route.py'],
    }
    outcome = "passed" if passed else "failed"
    raw = json.dumps(
        {"schema_version": 1, **identity(), "outcome": outcome, "checks": [check],
         "summary": "Provider-timeout fallback acceptance passed." if passed else "Provider-timeout fallback acceptance failed."},
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    ).encode("utf-8")
