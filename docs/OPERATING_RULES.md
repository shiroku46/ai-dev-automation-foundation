...
The record contains `notification: false`, no required human action, the reason, Issue, Pull Request, exact SHA, bounded detail, and connected audit evidence. The canonical public contract sets human_action_required: `false`. The path and exact content are the idempotency key. Routine internal stops are never posted as Issue or Pull Request comments and never create or edit routine stop labels. A deliberately disposable negative E2E Pull Request may close only after exact record persistence and another live-head check.
