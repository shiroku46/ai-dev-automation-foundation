...
HUMAN_ONLY_RISKS = {
    "secret", "permission", "repository-setting", "billing", "authentication",
    "deployment", "production", "destructive-data", "essential-ambiguity",
    "mfa", "captcha", "hardware-key", "provider-ui",
}
...
def _sorted_dict(value):
    ...
    if isinstance(value, (list, tuple, set)):
        converted = [_sorted_dict(v) for v in value]
...
def _decision(state, policy, action, reason, explanation):
...
def decide(state: State, policy: Policy = Policy()) -> Decision:
    ...
    if risks & HUMAN_ONLY_RISKS:
        return _decision(state, policy, Action.ESCALATE_HUMAN, Reason.HUMAN_ONLY,
                         "A genuine human-only risk is present after self-resolution.")

    changed = set(state.protected_paths_changed)
    authorized = set(state.protected_authorized_paths)
    if changed - authorized:
        return _decision(state, policy, Action.BLOCK_AND_CLOSE, Reason.UNAUTHORIZED_PROTECTED,
                         "Protected paths are not covered by trusted Issue authorization.")

    manifest = {c.context: c for c in state.checks if c.sha == state.head_sha}
    missing = [name for name in policy.required_checks if name not in manifest]
    if missing:
        return _decision(state, policy, Action.RERUN_EXACT_SHA_CHECKS, Reason.MISSING_CHECKS,
    ...
    for name in policy.required_checks:
        ...
        if check.producer not in policy.trusted_producers:
            return _decision(state, policy, Action.ESCALATE_HUMAN, Reason.HUMAN_ONLY,
                             f"Check {name} has an untrusted producer.")
        if check.state in {"queued", "pending", "in_progress"}:
            return _decision(state, policy, Action.WAIT, Reason.CHECKS_RUNNING,
                             f"Check {name} is still running.")

    failures = [manifest[name] for name in policy.required_checks if manifest[name].state != "success"]
    if failures:
        if state.transient_failure:
            if state.attempt_count >= policy.max_retries:
                return _decision(state, policy, Action.ESCALATE_HUMAN, Reason.EXHAUSTED,
                                 "Transient retry budget is exhausted.")
            if state.attempt_count > 0 and state.seconds_since_last_attempt is None:
                return _decision(state, policy, Action.WAIT, Reason.COOLDOWN,
            ...
            return _decision(state, policy, Action.RETRY_TRANSIENT, Reason.TRANSIENT,
        ...
        if state.deterministic_failure and state.bounded_fix:
            ...
            paths = set(fix.paths)
            if (valid_identity and paths and len(paths) <= policy.max_bounded_fix_paths
                    and paths <= set(state.allowed_fix_paths)):
                return _decision(state, policy, Action.REQUEST_BOUNDED_FIX, Reason.BOUNDED_FIX,
                                 "Concrete current failure is bound to a small allowlisted fix.")
        return _decision(state, policy, Action.ESCALATE_HUMAN, Reason.AMBIGUOUS,
                         "Failure is not safely repairable within a bounded current scope.")

    if not state.codex.reviewed or state.codex.sha != state.head_sha:
        return _decision(state, policy, Action.REQUEST_CODEX_REVIEW, Reason.CODEX_REQUIRED,
                         "Fresh Codex review is required for the current exact SHA.")
    if state.codex.blockers:
        return _decision(state, policy, Action.REQUEST_BOUNDED_FIX, Reason.CODEX_BLOCKER,
                         "Codex reported blocking findings on the current exact SHA.")

    if state.no_progress_seconds >= 3600:
        return _decision(state, policy, Action.ESCALATE_HUMAN, Reason.HUMAN_ONLY,
                         "No meaningful progress for sixty minutes; structured stop report required.")

    if state.draft:
        return _decision(state, policy, Action.MARK_READY, Reason.READY,
                         "All evidence gates pass; mark the exact-SHA Draft ready.")
    if state.mergeable:
        return _decision(state, policy, Action.MERGE, Reason.MERGE,
    ...
    if state.next_phase and state.next_phase.goal and state.next_phase.allowed_paths:
        return _decision(state, policy, Action.CREATE_NEXT_PHASE, Reason.NEXT_PHASE,
                         "A concrete predeclared next phase is available.")
    return _decision(state, policy, Action.WAIT, Reason.NOOP, "No safe state transition is available.")
